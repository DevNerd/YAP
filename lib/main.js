...
var panel = require("panel").Panel({
  width: 200,
  height: 180,
  contentURL: data.url("correct.html")
});
		...
		onComplete : function(response) {
			if(response.status == 200) { // 200 OK?
				if(CleanCrypto[extention] == response.text.split(" ")[0]) {
					panel.contentURL= data.url("valid.html");
					panel.show();
					//timer.initWithCallback(func, 10000, Components.interfaces.nsITimer.TYPE_ONE_SHOT) // HACKY SOLUTION
				} else {
					panel.contentURL= data.url("notvalid.html");
					panel.show();
					//imer.initWithCallback(func, 10000, Components.interfaces.nsITimer.TYPE_ONE_SHOT) // HACKY SOLUTION
				}
				// So, No hashes, lets try some less-safer methods.
					...
					if(https==1){
						...
						if(extention==hashtype.SHA1){
							...
							panel.contentURL= data.url("notfound.html")
							panel.show();
							timer.initWithCallback(func, 10000, Components.interfaces.nsITimer.TYPE_ONE_SHOT) // HACKY SOLUTION
						}
					}
