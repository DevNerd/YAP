...
function getHash(fileLocation, extention, https, CleanCrypto) {
	...
	if(extention=="md5"){
		CleanCrypto = CleanCrypto[hashtype.MD5]
	}else{
		CleanCrypto = CleanCrypto[hashtype.SHA1]
	}
	if(https == 1) {
		...
		onComplete : function(response) {
			if(response.status == 200) { // 200 OK?
				...
				// So, No hashes, lets try some less-safer methods.
					...
					if(https==1){
						if(extention=="sha1"){
							getHash(fileLocation, "md5",1)
						}else{
							getHash(fileLocation, "sha1",0)
						}
					}else{
						if(extention=="sha1"){
							getHash(fileLocation,"md5",0)
						}else{
...
var Downloadstate = { onDownloadStateChange : function(aOldState, aDownload) { if(Ci.nsIDownloadManager.DOWNLOAD_FINISHED == aDownload.state) {
			...
			var hash = []
			hash[hashtype.SHA1] = getHashFromStream(input_stream,hashtype.SHA1);
			hash[hashtype.MD5] = getHashFromStream(input_stream,hashtype.MD5);
			var CleanCrypto =[]
			CleanCrypto[hashtype.SHA1] = [HexToString(hash.charCodeAt(i)) for (i in hash[hashtype.SHA1])].join("");
			CleanCrypto[hashtype.MD5] = [HexToString(hash.charCodeAt(i)) for (i in hash[hashtype.MD5])].join("");
